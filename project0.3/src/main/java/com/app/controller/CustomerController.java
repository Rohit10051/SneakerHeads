...
public class CustomerController {
	...
	public ResponseDTO<?> deleteItemFromCart(@PathVariable String cartId){
	...
	@GetMapping("/orders-rent/{userId}/{totalPrice}")
	public ResponseDTO<?> addRentOrder(@PathVariable int userId, @PathVariable double totalPrice){
		System.out.println("in addOrder: "+userId);
		try {		
			return new ResponseDTO<>(HttpStatus.OK, "Order Added", custService.addRentOrder(userId, totalPrice));
		}catch (RuntimeException e) {
			System.out.println("err in addOrder : "+e);
			return new ResponseDTO<>(HttpStatus.INTERNAL_SERVER_ERROR, "Order Not Added", null);
		}
	...
	@GetMapping("/rentdetails/{userId}/{orderId}/{bookedDuration}/{productId}")
	public ResponseDTO<?> addRentDetails(@PathVariable int userId, @PathVariable int orderId, @PathVariable int bookedDuration, @PathVariable int productId){
		System.out.println("in addorderDetails: "+userId);
		try {		
			return new ResponseDTO<>(HttpStatus.OK, "order Details Added", custService.addRentDetails(userId, orderId, productId, bookedDuration));
		}catch (RuntimeException e) {
			System.out.println("err in addorderDetails : "+e);
			return new ResponseDTO<>(HttpStatus.INTERNAL_SERVER_ERROR, "Order Details Not Added", e);
		}
	}
	...
	public ResponseDTO<?> getAllCategoryList(){
		...
		try {		
			 List<Categories> list = custService.getAllCategoryList();
	...
	@GetMapping("/rentProductList")
	public ResponseDTO<?> getAllRentProducts(){
		System.out.println("in getAllRentProducts");
		try {		
			 List<Product> list = custService.getAllRentProducts();
			 System.out.println("Product list : "+list);
			return new ResponseDTO<>(HttpStatus.OK, "Product List sent",list);
		}catch (RuntimeException e) {
			System.out.println("err in getProductsForHomePage");
			return new ResponseDTO<>(HttpStatus.INTERNAL_SERVER_ERROR, "Category List Not found", null);
		}
	}
	
	@GetMapping("/buyProductList")
	public ResponseDTO<?> getAllBuyProducts(){
		System.out.println("in getAllBuyProducts");
		try {		
			 List<Product> list = custService.getAllBuyProducts();
			 System.out.println("Product list : "+list);
			return new ResponseDTO<>(HttpStatus.OK, "Product List sent",list);
		}catch (RuntimeException e) {
			System.out.println("err in getProductsForHomePage");
