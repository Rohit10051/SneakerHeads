    ...
    return (
       <div>
        ...
        <table className="table table-striped">
            ...
            <tbody>
                {/* <div className="container"><h5 className="nameColor1">{this.state.orders.filter(order => order.status === 'PENDING').length == 0 && this.state.message}</h5></div> */}
                {this.state.orders.map(
                        order =>
                        order.status === 'PENDING' && <tr key={order.id}>
                                <td>{order.id}</td>
                                <td>{order.totalPrice}</td>
                                <td>{order.orderDate}</td>
                                <td>{order.type}</td>
                                <td className="nameColor1"><h5>{order.status}</h5></td>
